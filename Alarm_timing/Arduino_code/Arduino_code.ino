...
int snooze=0,alarm_status=0,size_tone=0;
char delays[]={},frequency[] ={};
//void melody_1(int []);
void setup() {
...
void loop() {
  if(digitalRead(tone1)==0 ){alarm_status=1;
  //digitalWrite(8,HIGH);
  melody_1();}
  else if(digitalRead(tone2)==0 & alarm_status ==0){alarm_status=1;melody_2();}
}
...
void melody_1(){
  ...
  int divider = 0, noteDuration = 0;
    ...
    for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
      ...
      if (digitalRead(stop_alarm)==0){alarm_status=0;break;}
      else if(digitalRead(snooz)==0){delay(3000);
      //melody_1();
      }
      tone(speaker, melody1[thisNote], noteDuration * 0.9);    // we only play the note for 90% of the duration, leaving 10% as a pause
...
void melody_2(){
  ...
  int divider = 0, noteDuration = 0;
    ...
    for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
      ...
      tone(speaker, melody2[thisNote], noteDuration * 0.9);    // we only play the note for 90% of the duration, leaving 10% as a pause
      delay(noteDuration);                                     // Wait for the specief duration before playing the next note.
      noTone(speaker);                                          // stop the waveform generation before the next note.
    }
 if (alarm_status==1){
